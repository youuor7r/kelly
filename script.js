...
// 계산기 3 (이미지 기반 - f = p/a - q/b)
const winRateInput3 = document.getElementById('winRate3');
const winReturnInput3 = document.getElementById('winReturn3');
const lossReturnInput3 = document.getElementById('lossReturn3');
...
calculateBtn2.addEventListener('click', function() {
    ...
    const result = kellyFractionFromReturns(p, winReturn, lossReturn);
...
calculateBtn3.addEventListener('click', function() {
    const p = parseFloat(winRateInput3.value);
    const winReturn = parseFloat(winReturnInput3.value);
    const lossReturn = parseFloat(lossReturnInput3.value);

    if (isNaN(p) || isNaN(winReturn) || isNaN(lossReturn)) {
        showError('모든 입력값을 숫자로 입력해주세요.', resultDiv3);
        return;
    }
    
    if (winReturn <= 0 || lossReturn <= 0) {
        showError('수익률과 손실률은 0보다 커야 합니다.', resultDiv3);
        return;
    }
    
    if (p < 0 || p > 1) {
        showError('승률은 0과 1 사이의 값이어야 합니다.', resultDiv3);
        return;
    }

    const result = kellyFractionFromImage(p, winReturn, lossReturn);
    
    if (result.error) {
        showError(result.error, resultDiv3);
    } else {
        showResult3(result);
    }
});
...
function showResult3(result) {
    ...
    let html = `
        <div class="result-item">
            <h4>Kelly 비율 (f*)</h4>
            <div class="value">${(kellyFraction * 100).toFixed(2)}%</div>
            <div class="description">최적 투자 비중 (f = p/a - q/b 공식 사용)</div>
        </div>
    `;
...
setupEnterKey([winRateInput3, winReturnInput3, lossReturnInput3], calculateBtn3);
